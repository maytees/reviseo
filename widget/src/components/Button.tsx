...
const buttonVariants = cva(
	"inline-flex hover:cursor-pointer items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-all duration-200 outline-none focus-visible:ring-[3px] focus-visible:ring-ring/50 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0",
	{
		variants: {
			variant: {
				default:
...
export interface ButtonProps
	extends VariantProps<typeof buttonVariants>,
		preact.ButtonHTMLAttributes<HTMLButtonElement>,
...
function Button({
	...
	variant,
	size,
	// biome-ignore lint/a11y/useButtonType: biome trippin
	render = <button />,
	...props
}: ButtonProps) {
	const defaultProps = {
