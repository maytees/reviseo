...
const MacAppMockHeader = React.forwardRef<HTMLDivElement, MacAppMockHeaderProps>(
    ({ className, children, dark = false, ...props }, ref) => {
        const [isHovered, setIsHovered] = React.useState(false)
            ...
            <motion.div
                ...
                animate={{
                    backgroundColor: isHovered
                        ? (dark ? "rgba(24, 24, 27, 0.7)" : "rgba(0, 0, 0, 0.05)")
                        : (dark ? "rgba(24, 24, 27, 0.5)" : "rgba(0, 0, 0, 0.03)")
                }}
...
const MacAppMockContent = React.forwardRef<HTMLDivElement, MacAppMockContentProps>(
    ({ className, children, dark = false, ...props }, ref) => (
        <div ref={ref} className={cn("p-6", dark && "text-zinc-100", className)} {...props}>
            {children}
        </div>
    ),
)
